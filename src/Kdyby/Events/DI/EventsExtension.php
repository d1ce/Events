...
class EventsExtension extends Nette\DI\CompilerExtension
{
     ...
     * @var array
     */
    public $defaults = array('subscribers' => array(), 'validate' => TRUE, 'autowire' => TRUE, 'optimize' => TRUE, 'debugger' => '%debugMode%', 'exceptionHandler' => NULL, 'globalDispatchFirst' => FALSE);
    /**
     * @var array
     */
    ...
    public function loadConfiguration()
    {
        ...
        if (!array_key_exists('debugger', $userConfig)) {
            if (in_array(php_sapi_name(), array('cli', 'phpdbg'), TRUE)) {
        ...
        foreach ($config['subscribers'] as $i => $subscriber) {
			...
			$args = Nette\DI\Compiler::filterArguments(array(is_string($subscriber) ? new Nette\DI\Statement($subscriber) : $subscriber));
            $def->setFactory($args[0]);
...
', trim($init->getBody()));
    ...
    private function validateSubscribers(Nette\DI\ContainerBuilder $builder, Nette\DI\ServiceDefinition $manager)
    {
        foreach ($manager->getSetup() as $stt) {
            ...
            if (!$def->getClass()) {
                ...
                throw new AssertionException('Class \'' . $def->getClass() . '\' of ' . (is_numeric($serviceName) ? 'anonymous ' : '') . "service '{$serviceName}' cannot be found. " . 'Please make sure, that the class exists and can be autoloaded.');
            }
    ...
    {
        return $definition->getFactory() && ($definition->getEntity() instanceof Nette\DI\ServiceDefinition || is_string($definition->getEntity()) && substr($definition->getEntity(), 0, 1) === '@');
    }
    /**
     * @param \Nette\DI\ContainerBuilder $builder
     */
    ...
    protected function bindEventProperties(Nette\DI\ServiceDefinition $def, Nette\Reflection\ClassType $class)
    {
        foreach ($class->getProperties(Nette\Reflection\Property::IS_PUBLIC) as $property) {
            ...
            $def->addSetup('$' . $name, array(new Nette\DI\Statement($this->prefix('@manager') . '::createEvent', array(array($class->getName(), $name), new Code\PhpLiteral('$service->' . $name), NULL, $property->hasAnnotation('globalDispatchFirst') ? (bool) $property->getAnnotation('globalDispatchFirst') : $this->loadedConfig['globalDispatchFirst']))));
        }
    }
    /**
     * @param \Nette\DI\ContainerBuilder $builder
    ...
    private function optimizeListeners(Nette\DI\ContainerBuilder $builder)
    {
        ...
        $builder->getDefinition($this->prefix('manager'))->setClass('Kdyby\\Events\\LazyEventManager', array($listeners))->setSetup($this->allowedManagerSetup);
    }
    /**
