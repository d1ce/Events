...
class ExtensionTest extends Tester\TestCase
{
    ...
    public function testRegisterListenersWithSameArguments()
    {
        ...
        /** @var Kdyby\Events\EventManager $manager */
        Assert::true($manager instanceof Kdyby\Events\EventManager);
        Assert::same(array('onFoo'), array_keys($manager->getListeners()));
    ...
    public function testInherited()
    {
        ...
        Assert::same(array('KdybyTests\\Events\\LeafClass::onCreate' => 2), $subscriber->eventCalls);
        Assert::same(array('KdybyTests\\Events\\LeafClass::onCreate' => 1), $subscriber2->eventCalls);
    }
    public function testOptimize()
    {
        ...
        Assert::same(array(array('KdybyTests\\Events\\EventListenerMock::onFoo', array($bazArgs))), $bar->calls);
        Assert::same(array(array('KdybyTests\\Events\\NamespacedEventListenerMock::onFoo', array($bazArgsSecond))), $baz->calls);
    }
    public function testOptimize_dispatchNamespaceFirst()
    {
        ...
        $baz = $container->getService('baz');
        /** @var NamespacedEventListenerMock $baz */
        Assert::same(array(array('KdybyTests\\Events\\NamespacedEventListenerMock::onFoo', array($bazArgs))), $baz->calls);
    }
    public function testOptimize_standalone()
    {
        ...
        /** @var NamespacedEventListenerMock $baz */
        Assert::same(array(array('KdybyTests\\Events\\LoremListener::onStartup', array($bazArgs))), $baz->calls);
    }
