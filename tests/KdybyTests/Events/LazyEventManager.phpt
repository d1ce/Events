...
class LazyEventManagerTest extends Tester\TestCase
{
    public function dateGetListeners()
    {
        ...
        $lazy = new LazyEventManager(array('App::onFoo' => array('first'), 'onFoo' => array('second'), 'onBar' => array('second'), 'Article::onDiscard' => array('third'), 'onBaz' => array('fourth'), 'onQuux' => array('fifth')), $sl);
    ...
    public function testGetListeners_single(Container $sl, LazyEventManager $lazy)
    {
        ...
        Assert::same(array($sl->getService('fifth')), $quuxListener);
    }
    /**
     * @dataProvider dateGetListeners
     */
    public function testGetListeners_all(Container $sl, LazyEventManager $lazy)
    {
        ...
        Assert::true($sl->isCreated('fourth'));
        Assert::true($sl->isCreated('fifth'));
        Assert::same(array('App::onFoo' => array($sl->getService('first')), 'onFoo' => array($sl->getService('second')), 'onBar' => array($sl->getService('second')), 'Article::onDiscard' => array(array($sl->getService('third'), 'customMethod')), 'onBaz' => array($sl->getService('fourth')), 'onQuux' => array($sl->getService('fifth')), 'onCorge' => array($sl->getService('fifth'))), $all);
    }
    /**
     * @dataProvider dateGetListeners
     */
    public function testRemoveSubscriber(Container $sl, LazyEventManager $lazy)
    {
        ...
        $lazy->removeEventListener($fourth);
        // callable is listener, not subscriber
