...
class EventTest extends Tester\TestCase
{
    ...
    {
        return array(array(array(NULL, 'onFoo', NULL), 'onFoo'), array(array('App', 'onFoo', '::'), 'App::onFoo'), array(array('App', 'onFoo', '::'), '\\App::onFoo'), array(array('app.blog', 'foo', '.'), 'app.blog.foo'));
    }
    /**
    ...
    public function testDispatchOrderGlobalFirst()
    {
        ...
        Assert::same(array($args), $listener->calls[0][1]);
    ...
    public function testDispatchOrderGlobalLast()
    {
        ...
        Assert::same(array($args), $listener->calls[1][1]);
    }
}
\run(new EventTest());